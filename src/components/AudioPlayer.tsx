...
export default function AudioPlayer({ tracks = [] }: { tracks?: string[] }) {
  ...
  const crossfadeToNext = useCallback(() => {
    ...
    const nextAudio = state.activePlayer === 1 ? audioRef2.current : audioRef1.current;
    ...
    if (nextIndex >= state.playlist.length) {
      ...
      if (nextPlaylist.length > 1 && nextPlaylist[0] === state.playlist[state.playlist.length - 1]) {
    ...
    nextAudio.onloadedmetadata = () => {
      ...
      if (duration > 20) {
         ...
         nextAudio.currentTime = Math.random() * (maxStart - minStart) + minStart;
      }
  ...
  const startPlayback = useCallback(() => {
    ...
    activeAudio.onloadedmetadata = () => {
      ...
      if (duration > 20) {
         const minStart = duration * 0.3;
         const maxStart = duration * 0.7;
         activeAudio.currentTime = Math.random() * (maxStart - minStart) + minStart;
      }
  ...
  useEffect(() => {
    const handleInteraction = () => {
      const state = stateRef.current;
      if (state.userPaused || isPlaying || state.hasInteracted || state.playlist.length === 0) return;
    ...
    <>
      ...
      <div 
        ...
        className={`fixed inset-0 z-[9999999] bg-black flex flex-col items-center justify-center cursor-pointer transition-all duration-1000 ease-[cubic-bezier(0.7,0,0.3,1)] ${
          siteEntered ? 'opacity-0 scale-[1.5] pointer-events-none' : 'opacity-100 scale-100'
        }`}
      >
        <div className={`text-center transition-all duration-1000 ${siteEntered ? 'opacity-0 scale-[1.2]' : 'animate-[pulse_3s_ease-in-out_infinite]'}`}>
      ...
      <div className={`fixed bottom-6 right-6 z-[99999] transition-opacity duration-1000 ${siteEntered ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <audio ref={audioRef1} />
        <audio ref={audioRef2} />
        ...
        >
          {isPlaying ? (
            <Volume2 size={20} className="text-[#39FF14] group-hover:text-[#39FF14] transition-colors" />
          ) : (
            <VolumeX size={20} className="text-white/60 group-hover:text-white transition-colors" />
          )}
        </button>
      </div>
    </>
